...
### 2.5 Тех.долг (выявлен code review)
- [ ] Исправить русскую плюрализацию «отзывов» в ProductPage (21→"отзыв", 24→"отзыва")
- [ ] Добавить `Field(ge=1, le=5)` к `AddReviewRequest.rating` и `Field(min_length=1)` к `text`
- [ ] Экранировать LIKE-спецсимволы (`%`, `_`) в SQL color-фильтре
- [ ] Добавить `products` в зависимости `filtered` useMemo в CatalogPage (pre-existing баг)
