...
## Фаза 8: Авторизация и личный кабинет 🚧 В РАБОТЕ

### 8.1 Авторизация
- [ ] Страницы: Вход (email + пароль), Регистрация, Восстановление пароля
- [ ] Zustand auth store с persist (token, user, isAuth, login, register, logout)
- [ ] Protected routes — RequireAuth обёртка (редирект на /login)

### 8.2 Личный кабинет (AccountLayout + sidebar)
- [ ] Sidebar-навигация: Профиль, Заказы, Мои проекты, Конструктор, Избранное, Подписка
- [ ] Профиль: просмотр/редактирование данных (имя, email, телефон, адреса)
- [ ] История заказов: список, статусы (оформлен → в работе → доставлен → установлен)
- [ ] Сохранённые проекты конструктора (список с превью, загрузка в конструктор)
- [ ] Конструктор в ЛК: полноценный конструктор стен внутри аккаунта (авто-сохранение проектов)
- [ ] Избранные дизайны (сердечко на карточках каталога)
- [ ] Управление подпиской (текущий план, смена, отмена, история платежей)

### 8.3 Стор данных аккаунта
- [ ] accountStore.ts — Zustand + persist (orders, projects, favorites)
- [ ] Моковые данные для заказов и проектов
- [ ] Интеграция избранного в каталог (карточки товаров)
