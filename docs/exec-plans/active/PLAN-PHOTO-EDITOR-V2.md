...
| Mask canvas пересоздаётся при каждом движении слайдера opacity (useMemo + maskOpacity) | 2 | Средняя | Оптимизация: рендерить маску с opacity=1, управлять прозрачностью через Konva Image opacity — отложено до Фазы 5 |
| Brush cursor (Layer 5) статичен и не следует за мышью | 2 | Низкая | Требуется отдельный state для позиции мыши + onMouseMove обновление — отложено до Фазы 5 |
