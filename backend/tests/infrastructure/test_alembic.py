...
def test_upgrade_head_creates_all_core_tables(alembic_cfg):
    ...
    # head revision must equal the latest migration id (currently 015 — Phase 7A is_published).
    assert _current_revision(db_path) == "015"
    ...
    # 013 adds audit_entries append-only log + indexes — Phase 9.
    assert _table_exists(db_path, "audit_entries")
    assert _index_exists(db_path, "ix_audit_entries_actor_id")
