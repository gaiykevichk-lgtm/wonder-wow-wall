    ...
    >
      {items.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
          ...
          >
            <Button
              ...
              style={{ background: '#2D2D2D', borderRadius: 8 }}
            >
              Перейти в каталог
            </Button>
          </Empty>
        ...
        <>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontWeight: 600, color: '#2D2D2D' }}>{(item.price * item.quantity).toLocaleString('ru-RU')} &#8381;</span>
          ...
          <div style={{ padding: '20px 24px', borderTop: '1px solid rgba(0,0,0,0.04)', background: '#F5F5F5' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 16, fontWeight: 500, color: '#2D2D2D' }}>Итого:</span>
              <span style={{ fontSize: 22, fontWeight: 600, color: '#2D2D2D' }}>{cartTotal.toLocaleString('ru-RU')} &#8381;</span>
            </div>
            <Button
              type="primary"
              block
              ...
              style={{ background: '#4CAF50', height: 48, fontWeight: 600, fontSize: 15, borderRadius: 8 }}
            >
              Оформить заказ
            </Button>
          </div>
