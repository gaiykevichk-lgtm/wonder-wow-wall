...
export interface BrandedFrameProps {
  /** Show the corporate logo in the bottom-right corner. */
  variant?: 'full' | 'compact';
  /** Stroke colour for both rules. Defaults to the brand dark grey. */
  tone?: string;
  /** Page/card colour the frame is sitting on. Used to mask the inner
   *  rule under the logo. Set this when placing the frame on something
   *  other than white. */
  bgColor?: string;
  /** Inner padding from the inner border to children. Default 24 px. */
  padding?: number | string;
  /** Distance between outer and inner borders. Default 8 px. */
  gap?: number;
  /** Override the default 1 px stroke. */
  strokeWidth?: number;
  /** Border-radius (mild — defaults to 0 to match the printed asset). */
  borderRadius?: number;
  /** Fade + scale on mount. Respects `prefers-reduced-motion`. */
  animate?: boolean;
  /** Logo height in pixels (only used when `variant === 'full'`). */
  logoHeight?: number;
  /** Optional accent stripe colour applied to the logo's "W!" plate via
