...
const RestaurantIcon = ({
	...
	>
		...
		<path
			d="M20 30 Q20 18 40 18 Q60 18 60 30"
			stroke={color}
			strokeWidth="2.5"
			fill={color}
			fillOpacity="0.15"
		...
		<path
			d="M32 18 Q36 12 40 18 Q44 12 48 18"
			stroke={color}
			strokeWidth="2"
			fill="none"
		/>
...
const IconRenderer: React.FC<{ icon: string; color?: string }> = ({
	icon,
	color = "#4CAF50",
}) => {
	switch (icon) {
	...
	return (
		...
		>
			...
			>
				<IconRenderer
					icon={project.icon}
					color={hovered ? ACCENT : GRAY_TEXT}
				/>
				<Tag
					style={{
						position: "absolute",
...
const PortfolioPage: React.FC = () => {
	...
	return (
		<div style={{ fontFamily: FONT, paddingTop: 72 }}>
			<PageMeta
				title="Портфолио"
				description="Примеры реализованных проектов Wonder Wow Wall."
			/>
			{/* Hero */}
			<section style={{ background: "#F5F5F5", padding: "120px 24px" }}>
				<div style={{ ...MAX_WIDTH, textAlign: "center" }}>
					...
					>
						...
						>
							Мы создали коллекцию интерьеров для дома, бизнеса и общественных
							пространств. Меняйте фактуры, комбинируйте решения и находите свой
							WOW!
						</motion.p>
					</motion.div>
				</div>
