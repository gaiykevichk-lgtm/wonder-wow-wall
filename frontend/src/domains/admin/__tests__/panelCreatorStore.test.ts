...
function createTextureColorsMap(
	textureId: string,
	colors: ApiTextureColor[],
): Map<string, ApiTextureColor[]> {
...
describe("PanelCreatorStore", () => {
	...
	describe("design selection (step 1)", () => {
		it("setDesign stores selected design", () => {
			...
			expect(usePanelCreatorStore.getState().selectedDesign).toEqual(
				mockDesign,
			);
		});
		...
		it("setDesign replaces previous design", () => {
			...
			expect(usePanelCreatorStore.getState().selectedDesign?.id).toBe(
				"design-2",
			);
		});
	});
	...
	describe("texture selection (step 2)", () => {
		it("toggleTexture adds texture", () => {
			...
			expect(
				usePanelCreatorStore.getState().selectedTextureIds.has("tex-1"),
			).toBe(true);
		});
		...
		it("setTextures replaces selection", () => {
			...
			expect(
				usePanelCreatorStore.getState().selectedTextureIds.has("tex-2"),
			).toBe(true);
		});
	});
	...
	describe("size selection (step 3)", () => {
		it("toggleSize removes size if already selected", () => {
			...
			expect(usePanelCreatorStore.getState().selectedSizes.has("30x30")).toBe(
				false,
			);
		});
	...
	describe("variants (step 4)", () => {
		...
		it("setVariantImage updates variant image path", () => {
			...
			expect(usePanelCreatorStore.getState().variants[0].imagePath).toBe(
		...
		it("setVariantsImage updates multiple variants", () => {
			...
			expect(
				variants.every((v) => v.imagePath === "/images/batch-photo.jpg"),
			).toBe(true);
		});
