...
function RecommendationEditorDrawer({
	...
	const [newType, setNewType] = useState<RecommendationTargetTypeKey>("design");
					...
					>
						{draft.map((t, idx) => {
							...
							return (
								...
								>
									<Tag
										color={SOURCE_TAG_COLOR[t.target_type]}
										style={{ margin: 0 }}
									>
				...
				<div>
					...
					{isSelfReference && (
						<div style={{ marginTop: 6 }}>
							<Text type="danger">
								Источник не может рекомендовать сам себя.
							</Text>
						</div>
					)}
					{isDup && newId.trim() && (
						...
						>
							{(detail?.fallback_suggestions ?? []).map((s) => {
								...
								return (
									...
									>
										<Tag
											color={SOURCE_TAG_COLOR[s.target_type]}
											style={{ margin: 0 }}
										>
					...
					>
						...
						>
							...
							<div>
								<div>Дополнить</div>
								<div style={{ fontSize: 12, color: "#9CA3AF" }}>
									Добавить недостающие цели из источника без перезаписи
									существующих (с дедупликацией).
								</div>
							</div>
						</label>
					</div>
