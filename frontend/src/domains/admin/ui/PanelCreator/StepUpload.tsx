...
export function StepUpload({
	...
	const textureIdsNeedingImage = [
		...new Set(variantsNeedingImage.map((v) => v.textureId)),
	];
				...
				>
					...
					<Space wrap>
						{textureIdsNeedingImage.map((textureId) => {
							...
							const textureName = variants.find(
								(v) => v.textureId === textureId,
							)?.textureName;
							return (
								<Button
									key={textureId}
				...
				<div style={{ padding: "16px 0" }}>
					...
					{batchImagePath ? (
						<div>
							...
							>
								Применить ко всем вариантам (
								{batchTextureId
									? variants.filter(
											(v) => v.textureId === batchTextureId && !v.imagePath,
										).length
									: 0}
								)
							</Button>
