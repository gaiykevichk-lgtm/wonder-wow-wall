...
export function StepTextures({
	...
	const allSelected =
		textures.length > 0 && selectedIds.size === textures.length;
	const someSelected =
		selectedIds.size > 0 && selectedIds.size < textures.length;
	...
	return (
		<div>
			...
			>
				...
				<Text type="secondary">
					Выберите материалы, для которых будете загружать изображения
					комбинаций.
				</Text>
			</motion.div>
			...
			{textures.length === 0 && (
				<div style={{ textAlign: "center", padding: 48 }}>
					<Text type="secondary">
						Нет доступных текстур. Создайте их в разделе «Текстуры».
					</Text>
				</div>
			)}
		</div>
