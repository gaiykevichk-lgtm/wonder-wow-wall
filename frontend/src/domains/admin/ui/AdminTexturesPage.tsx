...
export default function AdminTexturesPage() {
			...
			.then((values) => {
				...
				if (editingTextureId === null) {
					createTexture.mutate(payload, {
						...
						onError: (err) =>
							handleTextureError(err, "Не удалось создать текстуру"),
					});
				} else {
					updateTexture.mutate(
						{
							textureId: editingTextureId,
							body: payload as TextureUpdatePayload,
						},
						{
							onSuccess: () => {
	...
	function onToggleTextureActive(t: ApiTexture, next: boolean): void {
		updateTexture.mutate(
			{ textureId: t.id, body: { is_active: next } },
			{
				...
				onError: (err) => handleTextureError(err, "Не удалось переключить"),
			},
		);
	}
	...
	const isTextureSubmitting =
		createTexture.isPending || updateTexture.isPending;
	...
	function handleColorError(err: unknown, fallback: string): void {
		if (
			...
			err.body?.code === "texture_color_not_found"
		) {
				...
				if (editingColorId === null) {
					createColor.mutate(
						{ textureId: selectedTextureId, body: payload },
						{
							...
							onError: (err) =>
								handleColorError(err, "Не удалось создать цвет"),
						},
					);
				} else {
					updateColor.mutate(
						{
							...
							body: payload as TextureColorUpdatePayload,
						},
						{
							onSuccess: () => {
								message.success("Изменения сохранены");
		...
		},
		{
			...
			render: (isActive: boolean, row) => (
				<Switch
					checked={isActive}
					loading={
						updateColor.isPending && updateColor.variables?.colorId === row.id
					}
	...
	return (
		...
		>
														...
														{existing ? (
															<div style={{ position: "relative" }}>
																...
																<Popconfirm
																	...
																	okButtonProps={{ danger: true }}
																	cancelText="Отмена"
																	onConfirm={() => onDeleteVariant(existing.id)}
																>
																	<Button
																		type="text"
																		danger
			...
			{/* ─── Color Drawer ────────────────────────────────────────────── */}
			<Drawer
				title={editingColorId === null ? "Новый цвет" : "Редактировать цвет"}
				styles={{ wrapper: { width: 480 } }}
				open={colorDrawer}
						...
						<Input
							...
							addonAfter={
								<ColorPicker
									...
									onChange={(_, hex) => colorForm.setFieldsValue({ hex })}
								/>
							}
						/>
					</Form.Item>
