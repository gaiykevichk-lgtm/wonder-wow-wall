...
export default function AdminCatalogPage() {
  ...
  const { data: categoriesData, error: categoriesError, isFetching: categoriesFetching } =
    useAdminCategories();
  const { data: designsData, error: designsError, isFetching: designsFetching } =
  ...
  function handleApiError(err: unknown, fallback: string): void {
    if (err instanceof ApiError) {
      ...
      if (code === 'category_slug_conflict' || code === 'design_slug_conflict') {
        const formInstance = drawerKind === 'category' ? categoryForm : designForm;
      ...
      >
        <Form<CategoryFormValues>
          ...
          onValuesChange={(changed) => {
            ...
            const currentSlug = (categoryForm.getFieldValue('slug') as string) ?? '';
      ...
      >
        <Form<DesignFormValues>
          ...
          onValuesChange={(changed) => {
            if (editingId !== null) return;
            if (!('name' in changed)) return;
            const currentSlug = (designForm.getFieldValue('slug') as string) ?? '';
            if (currentSlug === '' || currentSlug === lastAutoSlugRef.current) {
              const next = slugify(changed.name as string);
