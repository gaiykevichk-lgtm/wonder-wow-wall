...
const PlanCard: React.FC<{
  ...
  <motion.div variants={fadeUp} custom={index} style={{ position: 'relative' }}>
    <Card
      style={{
        borderRadius: 20,
        border: plan.popular ? `2px solid ${DARK}` : isActive ? `2px solid ${GREEN}` : '1px solid #E5E7EB',
    ...
    >
      {plan.popular && (
        <Tag style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: GREEN, color: '#fff', border: 'none', borderRadius: 20, fontFamily: FONT, fontWeight: 700, fontSize: 12, padding: '4px 16px', whiteSpace: 'nowrap' }}>
          Популярный
        </Tag>
      )}
      {isActive && (
        <Tag icon={<CrownOutlined />} style={{ position: 'absolute', top: -14, right: 16, background: GREEN, color: '#fff', border: 'none', borderRadius: 20, fontFamily: FONT, fontWeight: 700, fontSize: 12, padding: '4px 12px' }}>
          Ваш план
        </Tag>
      )}

      <div style={{ marginBottom: 24 }}>
        <h3 style={{ fontFamily: FONT, fontSize: 20, fontWeight: 700, color: plan.popular ? '#fff' : DARK, margin: '0 0 6px' }}>{plan.name}</h3>
      ...
      <Button
        ...
        disabled={isActive}
        style={{
          background: isActive ? '#E8E8E8' : plan.popular ? GREEN : DARK,
...
const PricingPage: React.FC = () => {
  ...
  return (
    <div style={{ fontFamily: FONT, paddingTop: 72 }}>
      ...
      <section style={{ background: '#F5F5F5', padding: '80px 24px' }}>
        <div style={{ ...MAX_WIDTH, textAlign: 'center' }}>
          <motion.div variants={stagger} initial="hidden" animate="visible" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <motion.h1 variants={fadeUp} custom={1} style={{ fontSize: 'clamp(36px, 4vw, 52px)', fontWeight: 800, color: DARK, margin: 0, lineHeight: 1.15 }}>
              Панели + Накладки
            </motion.h1>
      ...
      {hasSubscription() && (
        <section style={{ background: GREEN, padding: '16px 24px' }}>
          ...
          {activeTab === 'purchase' ? (
            <motion.div key="purchase" variants={stagger} initial="hidden" animate="visible" style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
                {panelPricing.map((item, i) => (
                  <motion.div key={item.size} variants={fadeUp} custom={i + 1}>
                    <Card style={{ borderRadius: 16, border: '1px solid #E5E7EB', height: '100%' }} styles={{ body: { padding: '28px' } }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                        <h3 style={{ fontSize: 22, fontWeight: 800, color: DARK, margin: 0 }}>{item.size}</h3>
                        ...
                        <div style={{ background: '#F9FAFB', borderRadius: 10, padding: '14px 16px' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, marginBottom: 8 }}>
                            ...
                            <span style={{ fontWeight: 600, color: DARK }}>
                              {hasSubscription() ? (
                                <><span style={{ textDecoration: 'line-through', color: '#9CA3AF', marginRight: 6 }}>{item.overlayPrice.toLocaleString('ru-RU')} ₽</span><span style={{ color: GREEN }}>0 ₽ (подписка)</span></>
                              ) : (
              ...
              <motion.div variants={fadeUp} custom={5} style={{ background: DARK, borderRadius: 16, padding: '28px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
                <div>
                  <p style={{ fontSize: 17, fontWeight: 700, color: '#fff', margin: '0 0 4px' }}>Рассчитайте стоимость вашей стены</p>
                ...
                <Button size="large" icon={<ArrowRightOutlined />} onClick={() => navigate('/constructor')} style={{ background: GREEN, color: '#fff', border: 'none', borderRadius: 10, height: 48, padding: '0 28px', fontWeight: 600, flexShrink: 0 }}>
                  Открыть конструктор
                </Button>
              </motion.div>
              ...
              <motion.div variants={fadeUp} custom={6} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
                {[
                  ...
                  <div key={item.title} style={{ padding: '20px', borderRadius: 12, border: '1px solid #E5E7EB' }}>
