...
const PricingPage: React.FC = () => {
	const [activeTab, setActiveTab] = useState<"purchase" | "subscription">(
		"purchase",
	);
	...
	return (
		<div style={{ fontFamily: FONT, paddingTop: 72 }}>
			<PageMeta
				title="Тарифы"
				description="Тарифные планы подписки Wonder Wow Wall."
			/>
			{/* Hero */}
			<section style={{ background: "#F5F5F5", padding: "120px 24px" }}>
				<div style={{ ...MAX_WIDTH, textAlign: "center" }}>
					...
					>
						...
						>
							Мы не продаём панели. Мы предоставляем доступ к интерьеру, который
							может меняться вместе с Вами.
						</motion.h1>
						<motion.p
							variants={fadeUp}
							custom={2}
						...
						>
							Базовая панель крепится на стену. Сверху — магнитная накладка с
							любым дизайном. Стоимость всех дизайнов одинакова:{" "}
							{designOverlayPrice.toLocaleString("ru-RU")} ₽ за накладку.
						</motion.p>
					...
					>
						...
						>
							...
							<span style={{ fontWeight: 600, fontSize: 14 }}>
								Активная подписка: {getActivePlan()?.name} — накладки включены в
								план (
								{getActivePlan()?.areaLimitM2
									? `${getActivePlan()!.areaLimitM2} м²/мес`
									: "безлимит"}
								)
							</span>
						</div>
					...
					{activeTab === "purchase" ? (
						...
						>
							<motion.div variants={fadeUp} custom={0}>
								...
								>
									Стоимость = панель + накладка. Панель покупается один раз.
									Накладки можно менять сколько угодно.
								</p>
							</motion.div>
												...
												>
													...
													>
														...
														<span style={{ fontWeight: 600, color: DARK }}>
															{(
																item.basePrice +
																(hasSubscription() ? 0 : item.overlayPrice)
							...
							>
								<div>
									...
									>
										Используйте конструктор — укажите размеры, выберите дизайн,
							...
							>
								{[
									{
										title: "Единая цена дизайна",
										text: "Любая накладка — одна цена. Никаких сюрпризов.",
									},
									{
										title: "Крепление на клипсах",
										text: "Быстрая смена дизайна без инструментов.",
									},
									{
										title: "Монтаж за 2 часа",
										text: "Профессиональная установка базовых панелей.",
									},
									{
										title: "100+ дизайнов",
										text: "Дерево, камень, абстракция, геометрия и другие.",
									},
								].map((item) => (
									<div
										key={item.title}
						...
						>
							<motion.div variants={fadeUp} custom={0}>
								...
								>
									Базовые панели покупаются один раз. По подписке вы получаете
									накладки на определённую площадь — меняйте дизайн когда
									захотите.
								</p>
							</motion.div>
							...
							>
								...
								>
									{[
										...
										"2. Выберите план подписки исходя из площади стен",
										"3. Каждый месяц получайте накладки на выбранную площадь с любым дизайном",
