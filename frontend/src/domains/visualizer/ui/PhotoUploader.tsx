	...
	const handleFile = useCallback(
		(file: File) => {
			if (!file.type.startsWith("image/") && !file.name.match(/\.heic$/i)) {
				onError?.("Выберите изображение (JPEG, PNG, WebP)");
				return;
			}
			onUpload(file);
		},
	...
	return (
		...
		>
			...
			<Text type="secondary" style={{ fontSize: 12 }}>
				JPEG, PNG, WebP, HEIC. Максимум 20 МБ. Минимум 600×450 px.
			</Text>
			...
			>
				<Text style={{ fontSize: 13, color: "#6B7280" }}>
					...
					<br />— Фотографируйте стену фронтально
					<br />— Обеспечьте хорошее освещение
					<br />— Захватите всю стену целиком
				</Text>
			</div>
		</motion.div>
	);
