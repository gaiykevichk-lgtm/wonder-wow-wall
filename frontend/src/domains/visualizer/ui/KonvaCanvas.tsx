...
import type {
	...
	EditorMode,
	CalibrationPoints,
	PerspectiveCorners as PerspectiveCornersType,
} from "../model/types";
...
interface KonvaCanvasProps {
	...
	/** URL of AI-generated preview image (base64 data URL) */
...
export function KonvaCanvas({
	...
	const [aiPreviewImage, setAiPreviewImage] = useState<HTMLImageElement | null>(
		null,
	);
	...
	// ─── Load AI preview image ───────────────────────────────────────────
	// Load/reload when aiPreviewUrl changes using a ref to capture latest value
	const aiPreviewUrlRef = useRef<string | null>(aiPreviewUrl);
	aiPreviewUrlRef.current = aiPreviewUrl;
	useEffect(() => {
		const url = aiPreviewUrlRef.current;
		if (!url) {
		...
		const img = new window.Image();
		const handleLoad = () => {
			if (aiPreviewUrlRef.current === url) {
	...
	const [panelImages, setPanelImages] = useState<Map<string, HTMLImageElement>>(
		new Map(),
	);
	useEffect(() => {
	...
	const scale = fitScale * zoom;
	const isPanMode =
		!maskTool && placementMode !== "accent" && editorMode === "default";
	...
	return (
		...
		>
			...
			>
				{/* Layer 1: Photo (show AI preview if available) */}
				<Layer>
					{(aiPreviewImage ?? photoImage) && (
						<KonvaImage
							image={aiPreviewImage ?? photoImage ?? undefined}
				...
				<Layer>
					{panels.map((panel) => {
						...
						const shadowCol =
							isSelected || isHovered ? "rgba(0,0,0,0.1)" : "rgba(0,0,0,0.04)";
						...
						if (perspectiveTransform) {
							...
							const designImg = panel.designImage
								? panelImages.get(panel.designImage)
								: null;
