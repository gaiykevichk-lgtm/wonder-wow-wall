...
export function OnboardingTooltips({ isReady }: OnboardingTooltipsProps) {
	...
	useEffect(() => {
		...
		maskToolbarRef.current = document.querySelector(
			'[data-testid="mask-toolbar-trigger"]',
		);
		panelPickerRef.current = document.querySelector(
			'[data-testid="panel-picker"]',
		);
		placementRef.current = document.querySelector(
	...
	const steps: TourProps["steps"] = [
		{
			...
			description:
				"Подправьте маску стены кистью, если автоматическое распознавание неточно.",
			target: () => maskToolbarRef.current as HTMLElement,
		...
		},
		{
			...
			target: () => panelPickerRef.current as HTMLElement,
			nextButtonProps: { children: "Далее" },
		...
		},
		{
			...
			description:
				"Нажмите «Авто» для автозаполнения стены или размещайте панели вручную.",
			target: () => placementRef.current as HTMLElement,
