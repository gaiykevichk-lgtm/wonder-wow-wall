...
import { trapezoidFromMaskBbox } from "../lib/maskUtils";
import { createPerspective } from "../lib/perspectiveEngine";
import {
	uint8ArrayToBase64,
	base64ToUint8Array,
} from "../lib/maskSerialization";
import {
	detectFromLines,
	type LineProvider,
...
interface VisualizerState {
	...
	setSegmentationStatus: (
		status: Scene["segmentationStatus"],
		error?: string,
	) => void;
...
export const useVisualizerStore = create<VisualizerState>()(
	persist(
		(set, get) => ({
			...
			setSegmentationProgress: (percent) =>
				set({ segmentationProgress: percent }),
			setCalibration: (calibration) => {
				...
				if (scene) {
					...
					set({
						scene: { ...scene, calibration, calibrationAutoDetected: false },
					});
				}
			},
			...
			setSelectedColor: (hex, name) =>
				set({ selectedColor: hex, selectedColorName: name }),
			...
			autoFill: () => {
				...
				if (!scene?.wallMask) {
					message.warning(
						"Сначала загрузите фото и дождитесь распознавания стены",
					);
					return;
				}
				if (!selectedDesignId) {
				...
				try {
					...
					if (panels.length === 0) {
						...
						message.info(
							"Не удалось разместить панели — недостаточно места на стене",
						);
						return;
					}
			...
			runAutoPerspective: async (provider, options) => {
				...
				const runStages = async (): Promise<void> => {
					...
					try {
						...
						if (!stillCurrent()) return;
						const vpResult = detectFromLines({
							lines,
							mask: wallMask,
							photoSize,
						});
				...
				try {
					...
					if (
						current &&
						current.segmentationStatus === "detecting-perspective"
					) {
			...
			getProjectPayload: () => {
				...
				return {
					...
					perspective_auto_detected:
						state.scene.perspectiveAutoDetected ?? false,
					calibration_auto_detected:
						state.scene.calibrationAutoDetected ?? false,
