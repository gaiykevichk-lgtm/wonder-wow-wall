...
| TanStack Query | 5.91 | Data fetching (интегрировано с Backend API) |
...
│   │
│   ├── api/                      # TanStack Query hooks + adapters для Backend API
...
### Домен visualizer — VisualizerStore (`domains/visualizer/model/visualizerStore.ts`)
...
API интегрирован. Vite proxy `/api` → `http://localhost:8080`. Переменные окружения:
